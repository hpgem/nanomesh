...
from nanomesh.mesh2d import (add_edge_points, add_speckle_grid,
                             add_speckle_kmeans, generate_2d_mesh,
                             subdivide_contour)
...
def test_generate_2d_mesh(segmented):
    ...
    mesh = generate_2d_mesh(segmented, pad=True, speckle=True, plot=True)
...
def test_add_edge_points():
    ...
    ret = add_edge_points(image, n_points=5)
...
def test_add_speckle_grid():
    """Test grid method for adding points."""
    image = np.zeros((10, 10))
    image[:5, :5] = 1
    image[-5:, -5:] = 1

    ret = add_speckle_grid(image, border=1, n_points=4)
...
def test_add_speckle_kmeans():
    ...
    np.random.seed(9)
    ret = add_speckle_kmeans(image, iters=5, n_points=10)
